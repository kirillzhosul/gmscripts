...
if (create_function == instance_create_depth and is_created){
	...
	if (is_created){
		logging_log(LOGGING_LOG_LEVEL.INFO, "`oScripts` successfully created objects on layer!");
	}else{
		logging_log(LOGGING_LOG_LEVEL.INFO, "`oScripts` skipped creating objects on layer, as they already created!");
	}
}
